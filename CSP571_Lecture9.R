...
# Let's load in a data set related to risk factors associated with
# giving birth to a baby that has extremely low birth-weight.
# Data set description can be found here:
# http://www.umass.edu/statdata/statdata/data/lowbwt.txt
# Tutorial adopted from: http://rstudio-pubs-static.s3.amazonaws.com/2899_a9129debf6bd47d2a0501de9c0dc583d.html
...
# Let's take a look at the full model and the null model
...
# Let's manually implement backward selection, using the F-test
# as our model evaluation criteria. We will sequentially drop
# variables until all variables are significant at a given cutoff, alpha
alpha <- .01
...
# Let's repeat
# We can do this more efficiently using the drop1 command.
...
# Let's do this with forward selection
...
# In practice, we do not do this manually.
library(rms)
ols.full <- ols(bwt ~ age + lwt + race.cat + smoke + preterm + ht + ui + ftv.cat, data = lbw)
fastbw(ols.full, rule = "p", sls = 0.1)


# Let's run through another example with simulated data
# Let's compare all the tecniques we have seen so far.
...
lmridge$kLW
lmridge$GCV
plot(lmridge, plottype="coefficients")
plot(lmridge, plottype="Cp")
